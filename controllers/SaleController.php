...
class SaleController extends Controller
{
    ...
    {
        if (Yii::$app->user->identity->user != 'Administrator') {
    ...
    public function actionGetMonthlySale($year)
    {
        $sale = [];
        for ($i = 1; $i <= 12; $i++) { 
            $sale[] = Sale::getMonthlySale($i, $year);
        }
        echo json_encode($sale);
    }

    public function actionReport($search, $year)
    {
        if (Yii::$app->user->identity->user != 'Administrator') {
            return $this->redirect(['/dashboard']);
        }
